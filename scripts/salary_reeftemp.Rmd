...
reef_colors <- c("Lagoon" = "darkgreen", "Reef Flat" = "darkorange",
                               "Forereef" = "maroon1", "Backreef" = "steelblue","Reef Crest"="firebrick")
...
#downlaod sat data from Copernicus Sentinel 2 (open use, CC BY)
sat_rast <- terra::rast(here("data/mada_sentinel.tiff"))
df <- as.data.frame(sat_rast, xy = TRUE, na.rm = TRUE)

names(df) <- c("x","y","red","green","blue")

#  scale to 0-255 since 32 bit
df$red   <- scales::rescale(df$red, to = c(0, 255))
df$green <- scales::rescale(df$green, to = c(0, 255))
df$blue  <- scales::rescale(df$blue, to = c(0, 255))
...
site_map <- ggplot() +
  geom_raster(data = df, aes(x = x, y = y),
              fill = rgb(df$red, df$green, df$blue, maxColorValue = 255)) +
      geom_spatvector(data = fishing_zones, aes(fill = Reef_zone),alpha=0.7) +
  ...
  labs(x = "Longitude", y = "Latitude",
       fill = "Geomorphic\nZone", 
       #color = "Recovery Status", size = "Recovery Status") +
  ) +
  coord_sf(xlim = c(min(df$x), max(df$x)), ylim = c(min(df$y), max(df$y)), expand = FALSE) +
  scale_fill_manual(values = reef_colors) +
...
# Add inset to your main site_map
site_map +
  geom_plot(data = df_inset, aes(x = x, y = y, label = plot))

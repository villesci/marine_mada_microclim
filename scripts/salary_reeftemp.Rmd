...
#make directory to hold figures
dir.create(here("figures"))

site_data<-read.csv(here('data/site_data.csv'))
fishery_sites<-read.csv(here('data/fishing_coords.csv'))
towns<-read.csv(here('data/towns.csv'))
#fishing zone shapefiles
fishing_zones<-st_read(here('data/fishing_site_spatial/Salary_Fishing_Sites_CPUE_March_2026.shp'))
#relevel some zones
fishing_zones <- fishing_zones %>%
  mutate(Reef_zone = forcats::fct_recode(Reef_zone, 
                                "Reef Flat" = "Inshore", "Reef Crest" = "Reef crest"))%>%
  mutate(Reef_zone = forcats::fct_relevel(Reef_zone, "Forereef", "Reef Crest", "Backreef", "Lagoon", "Reef Flat"))

#and for the sites as well
site_data <- site_data %>%
  mutate(zone = forcats::fct_relevel(zone, "Forereef", "Reef Crest", "Backreef", "Lagoon", "Reef Flat"))

reef_colors <- c("Forereef" = "maroon1","Reef Crest"="firebrick", "Backreef" = "steelblue","Lagoon" = "darkgreen", "Reef Flat" = "darkorange")

logger_colors <-  c("SALEB1 Shallow Lagoon" = "mediumorchid", "SALEB3 Backreef" = "steelblue", "SALEB5 Deep Lagoon" = "forestgreen", "SALEB8 Reef Flat" = "darkorange")
...
#downlaod sat data from Copernicus Sentinel 2 (open use, CC BY) - date of image 2025-07-19
sat_rast_full <- terra::rast(here("data/mada_sentinel_full.tiff"))
df_full <- as.data.frame(sat_rast_full, xy = TRUE, na.rm = TRUE)

names(df_full) <- c("x","y","red","green","blue")

#  scale to 0-255 since 32 bit
df_full$red   <- scales::rescale(df_full$red, to = c(0, 255))
df_full$green <- scales::rescale(df_full$green, to = c(0, 255))
df_full$blue  <- scales::rescale(df_full$blue, to = c(0, 255))
...
legend_key <- data.frame(
  ...
  type = factor(c("Logger", "Fishing Zone"), levels = c("Logger", "Fishing Zone"))
)

site_map <- ggplot() +
  geom_raster(data = df_full, aes(x = x, y = y),
              fill = rgb(df_full$red, df_full$green, df_full$blue, maxColorValue = 255)) +
  
  geom_spatvector(data = fishing_zones, aes(fill = Reef_zone), alpha = 0.7, color = "black", lwd = .5) +
  
  #geom_sf(data = towns_sf, color = "black", shape = 22, fill = "white",size=1) +
  #geom_sf_text(data = towns_sf, aes(label = town), color = "white", nudge_x = 0.02) +
  
  geom_sf(data = site_data_noair_sf %>% dplyr::filter(recovered == "TRUE"),
          aes(fill = zone), 
          shape = 24, stroke = .75, size = 2, show.legend = TRUE, color = "white") +
  ggrepel::geom_label_repel(
    data = site_data_noair_sf %>% dplyr::filter(recovered == "TRUE"),
    aes(label = id, geometry = geometry,fill=zone),
    stat = "sf_coordinates",
  ...
  coord_sf(xlim = c(min(df_full$x), max(df_full$x)),
           ylim = c(min(df_full$y), max(df_full$y)),
           expand = FALSE) +
  
  scale_fill_manual(
    values = reef_colors, 
    breaks = c("Forereef", "Reef Crest", "Backreef", "Lagoon", "Reef Flat"), 
    drop = FALSE,
    guide = guide_legend(
      override.aes = list(
        shape = c(NA, NA, 24, 24, 24), 
        color = c("black", "black", "black", "black", "black"),
        fill = reef_colors
      )
    )
  ) +
  annotation_scale(location = "br", bar_cols = c("darkgray", "white"), text_col = "white") +
  annotation_north_arrow(location = "tr", which_north = "true",
                         style = north_arrow_fancy_orienteering(fill = c("darkgray", "white"),
                                                                line_col = "black",
                                                                text_col = "white"))
...
mad.sp<-st_as_sf(MAD,IDs = MAD$names,proj4string = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
mad.ext <- terra::ext(42.143555,50.976563,-26.588527,-11.070603)
#mat.sp.crop<-terra::crop(mad.sp,mad.ext)
mad.st <- sf::st_crop(mad.sp, 
                      xmin = 42.143555, xmax = 50.976563, 
                      ymin = -26.588527, ymax = -11.070603)#load all sst data
sstmean<-load_layers("BO_sstmean")

mad.crop.sst <- terra::crop(terra::rast(sstmean), mad.ext)
...
inset <- mad.crop.sst %>%
  ...
  geom_sf(data = mad.st, inherit.aes = FALSE, fill = "antiquewhite") +
  theme_void() +
  labs(x = NULL, y = NULL, fill = "Mean SST\n(°C)") +
  geom_point(aes(x = mean(lon_range), y = mean(lat_range)), color = "black", size = 2, shape = 8, inherit.aes = FALSE) +
...
svg(file=here("figures/sitemap_raw.svg"),height=6,width=9)
...
png(file=here("figures/sitemap_raw.png"),res=300,units="in",height=6,width=9)
#plot_grid(aaa,site_map,rel_widths=c(1,.2))
site_map +
  geom_plot(
    data = df_inset, 
    aes(x = x, y = y, label = plot),
    vp.width = 0.45,   # Expanded from .3 to .45 to give the right-aligned legend room to draw
    vp.height = 0.35,  # Height allocation for the inset square
    hjust = 0,         # Left-aligns container on your 'x' coordinate
    vjust = 1          # Top-aligns container on your 'y' coordinate
  )

dev.off()


#map of all fishery sites used to analyze fisheries catch


site_map_allsites <- ggplot() +
  geom_raster(data = df_full, aes(x = x, y = y),
              fill = rgb(df_full$red, df_full$green, df_full$blue, maxColorValue = 255)) +
  ...
  geom_sf(data = site_data_noair_sf,
          ...
          shape = 21, stroke = 1, size = 3, show.legend = TRUE) +
  ggrepel::geom_label_repel(
    data = site_data_noair_sf ,
    aes(label = id, geometry = geometry,color = recovered),
    stat = "sf_coordinates",
    min.segment.length = 0,
    box.padding = 0.5,
    xlim = c(43.2, 43.32),
  ...
  labs(
    x = "Longitude", y = "Latitude",
    fill = "Reef\nZone",color = "Recovery Status", size = "Recovery Status") +
  scale_fill_manual(
    values = reef_colors, 
    breaks = c("Forereef", "Reef Crest", "Backreef", "Lagoon", "Reef Flat"), 
    drop = FALSE,
    guide = guide_legend(
      override.aes = list(
        shape = c(21, NA, 21, 21, 21), 
        color = c("darkgray", "black", "black", "black", "black"),
        fill = reef_colors
      )
    )
  ) +  
  scale_color_manual(values = c("FALSE" = "darkgray", "TRUE" = "black"), drop = FALSE) +
...
svg(file=here("figures/sitemap_full.svg"),height=9,width=6)
#plot_grid(aaa,site_map,rel_widths=c(1,.2))
site_map_allsites
dev.off()

png(file=here("figures/sitemap_full.png"),res=300,units="in",height=9,width=6)
#plot_grid(aaa,site_map,rel_widths=c(1,.2))
site_map_allsites
dev.off()
```
...
#data readin
eb.files<-list.files(here('data/logger_data'))
...
for (i in 1:length(eb.files)){
  log<-read.csv(paste0(here("data/logger_data",eb.files[i])),
...
##########################################
  ...
  write.csv(clim_point_allair,here('data/clim_point_allair.csv'))

```

```{r,echo=F,warning=F}

clim_point_allair<-read.csv(here('data/clim_point_allair.csv'))
...
ggsave(here("figures/airtemp_correlation.png"),air_cor,width=6,height=4,bg="transparent")
```
...
#####################################################################
write.csv(sat_sst,here("data/sat_sst.csv"))
```
...
cop_mada<-terra::rast(here("data/cop_sst.nc"))
...
#  OISST
sat_sst <- read.csv(here("data/sat_sst.csv")) %>%
  dplyr::select(-X) %>%
...
ggsave(here("figures/sat_logger_daily_correlations_supp.png"),height=9,width=6)
...
ggsave(here("figures/sat_logger_hourly_correlations_supp.png"),height=9,width=6)
...
svg(here("figures/water_temps_daily.svg"),width=6,height=8)
...
svg(file=here("figures/sitetempswater.svg"),height=6,width=12)
...
posthoc_daily<-dunn_test(temp_long, temp ~ id,p.adjust.method="bonferroni")

posthoc_daily <- posthoc_daily%>%
  rename("Logger 1" = group1,
         "Logger 2" = group2,
         "n1" = n1,
         "n2" = n2,
...
posthoc_daily <- posthoc_daily %>%
  mutate(
    ...
    `Adjusted P-value` = case_when(
      ...
      TRUE ~ as.character(round(`Adjusted P-value`, 4))
    )
  )

# View the result
posthoc_daily
posthoc_daily <- signif_df(posthoc_daily,3)
...
daily_data_table
write.csv(daily_data_table,here("data/daily_data_table.csv"))
```
...
posthoc_watertemps<-dunn_test(temp ~ id,data=water_temps)

posthoc_watertemps_nice<-kable(posthoc_watertemps[,-c(1,9)], digits=4,col.names = c("Logger 1","Logger 2","n1","n2","<i>Z</i> Statistic","P-value","Adjusted P-value"))
posthoc_watertemps_nice

#rename columns
posthoc_watertemps <- posthoc_watertemps %>%
  rename("Logger 1" = group1,
         "Logger 2" = group2,
         "n1" = n1,
         "n2" = n2,
...
# View the result
posthoc_watertemps
posthoc_watertemps <- signif_df(posthoc_watertemps,3)

  #adjust sig figs

write.csv(posthoc_watertemps[,-c(1,9)],here("data/posthoc_watertemps.csv"))
```
...
daily_vars_posthoc <- daily_vars_waterlogs %>%
  ungroup() %>%
  dunn_test(range ~ id)

daily_vars_posthoc <- daily_vars_posthoc%>%
  rename("Logger 1" = group1,
         "Logger 2" = group2,
         "n1" = n1,
         "n2" = n2,
...
# View the result
daily_vars_posthoc
posthoc_watertemps <- signif_df(daily_vars_posthoc,3)

  #adjust sig figs

write.csv(daily_vars_posthoc[,-c(1,9)],here("data/daily_vars_posthoc.csv"))
```
...
#plotted predictions
mada_tide23<-read.csv(here('data/tide/tide_prediction_-22.5512N_43.2810_2023.csv'))
mada_tide24<-read.csv(here('data/tide/tide_prediction_-22.5512N_43.2810_2024.csv'))
...
tide_height<-read.csv(here('data/tide_height_table.csv'))
...
ggsave(here("figures/saleb8_flat_tide_status.svg"), width = 6, height = 4, bg="transparent")
...
ggsave(here("figures/residuals_flat.png"), width = 8, height = 8, bg="white")
```
...
svg(here("figures/emersion_freq.svg"),width=4,height=4)
...
png(here("figures/wavelet_salary_flat.png"), width = 8, height = 6,bg="white", units="in",res=300) 
...
```{r,echo=F,warning=F}
#2x 
png(here("figures/wavelet_supp.png"), width = 12, height = 12,bg="white",units="in",res=300)  
...
saveRDS(coherency_saleb_1_5,here("data/coherency_saleb_1_5.rds"))

saveRDS(coherency_saleb_1_8,here("data/coherency_saleb_1_8.rds"))
```

```{r,echo=F,warning=F}

coherency_saleb_1_5<-readRDS(here("data/coherency_saleb_1_5.rds"))
coherency_saleb_1_8<-readRDS(here("data/coherency_saleb_1_8.rds"))
...
write.csv(all_tide_data_b,here("data/all_tide_data_18.csv"))
...
all_tide_data$datetime_posix<-as.POSIXct(all_tide_data$datetime_posix,format="%Y-%m-%d %H:%M:%S",tz="Africa/Addis_Ababa")+lubridate::hours(3)#convert from UTC to EAT
 ...
 write.csv(hours_100,here("data/hours_100.csv"))
 ...
 write.csv(min_ht_months,here("data/min_ht_months.csv"))
...
hours_100_daynight<-all_tide_data%>%
  ...
  summarize(hours = sum(tide_height < -100, na.rm = TRUE) / 60, .groups = "drop")
write.csv(hours_100_daynight,here("data/hours_100_daynight.csv"))
 ...
 min_ht_months_daynight<-all_tide_data%>%
  mutate(year = year(datetime), 
   ...
   group_by(year,month,day_night)%>%
    summarize(min = min(tide_height))
 write.csv(min_ht_months_daynight,here("data/min_ht_months_daynight.csv"))
...
hours_100<-read.csv(here("data/hours_100.csv"))
hours_100$month <- factor(hours_100$month, 
                                   levels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", 
                                              "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"))

min_ht_months<-read.csv(here("data/min_ht_months.csv"))
min_ht_months$month <- factor(min_ht_months$month, 
                                   levels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", 
                                              "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"))

hours_100_daynight<-read.csv(here("data/hours_100_daynight.csv"))
hours_100_daynight$month <- factor(hours_100_daynight$month, 
                                   levels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", 
                                              "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"))

min_ht_months_daynight<-read.csv(here("data/min_ht_months_daynight.csv"))
...
hours_below_season
ggsave(here("figures/hours_below_season.svg"),
...
ggsave(here("figures/month_exposure_comparison.svg"),
...
ggsave(here("figures/season_exposure_comparison.svg"),
...
all_tide_data<-read.csv(here("data/all_tide_data_18.csv"))

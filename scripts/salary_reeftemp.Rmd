...
fishing_zones <- fishing_zones %>%
  mutate(Reef_zone = forcats::fct_recode(Reef_zone, 
                                "Reef Flat" = "Inshore", "Backreef" = "Reef crest"))
...
site_map <- ggplot() +
  geom_raster(data = df, aes(x = x, y = y),
      ...
      geom_spatvector(data = fishing_zones, aes(fill = Reef_zone),alpha=0.4) +
  ...
  ggrepel::geom_label_repel(
    ...
    aes(label = id, color = recovered, geometry = geometry),
    stat = "sf_coordinates",
  ...
  labs(x = "Longitude", y = "Latitude",
       fill = "Zone", color = "Recovery Status", size = "Recovery Status") +
  coord_sf(xlim = c(min(df$x), max(df$x)), ylim = c(min(df$y), max(df$y)), expand = FALSE) +
  scale_fill_manual(values = c("Lagoon" = "green", "Reef Flat" = "red",
                               "Forereef" = "purple", "Backreef" = "blue")) +
...
pdf(file=here("figures/sitemap_raw.pdf"),height=9,width=9)
